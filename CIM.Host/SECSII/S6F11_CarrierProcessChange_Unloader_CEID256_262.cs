...
{
    class S6F11_CarrierProcessChange_Unloader_CEID256_262 : SFMessage
    {
        /*S6F11(CarrierProcessChange_Unloader), CEID:256, 257, 258, 259, 260, 261, 262*/
        private string _cellPortNo;

        public S6F11_CarrierProcessChange_Unloader_CEID256_262(SECSDriverBase driver, string CellPortNo)
            : base(driver)
        {
        ...
        public override void DoWork(string driverName, object obj)
        {
            ...
            string ceidTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CEID", module.MODULE_NAME);
            string carrierIDTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierID", module.MODULE_NAME);
            string carriertypeTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierType", module.MODULE_NAME);
            string carrierproductTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierProduct", module.MODULE_NAME);
            string carrierstepIDTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierStepID", module.MODULE_NAME);
            string carrierscountTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierSCount", module.MODULE_NAME);
            string carrierccountTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CarrierCCount", module.MODULE_NAME);
            string carrierportNoTagName = string.Format("i{0}.CarrierProcessChangeUnloader.PortNo", module.MODULE_NAME);

            string subcarrierIDTagName = string.Format("i{0}.CarrierProcessChangeUnloader.SubCarrierID1", module.MODULE_NAME);
            string cellQTYTagName = string.Format("i{0}.CarrierProcessChangeUnloader.CellQTY", module.MODULE_NAME);
            ...
            string sSUBCARRIERID = DataManager.Instance.GET_STRING_DATA(subcarrierIDTagName, out bResult); //KTW 180425
            string sCELLQTY = "0";
            ...
            if (sCARRIERTYPE != "")
            {
                ...
                CommonData.Instance.UNLOADER_CARRIERTYPE = sCARRIERTYPE;
            }
            ...
            */
                ...
                msg.AddList(3);
                {
                    ...
                    msg.AddList(2);
                    {
                            ...
                            sCARRIERTYPE == ((int)CarrierType.ForcedNotUseBatchID).ToString())
                        {
                            ...
                            CommonData.Instance.UNLOADER_CARRIERID = sCARRIERID;
                            ...
                            {//Standard Loader Carrier Assign Request(CEID:260) 빈 Tray Batch Assign 요청
                                ...
                                //group의 의미가 뭔지 모르겠음
                                List<string> SCID = CommonData.Instance.GetSubCarrierIDs(module.MODULE_NAME, "UnloaderSubCarrierID", nTrayQty);
                                 ...
                                 sCARRIERTYPE == ((int)CarrierType.ForcedNotUseTrayID).ToString())
                        {
                            ...
                            msg.AddList(1);
                            {
                                msg.AddList(3);
                                {
                                    ...
                                    CommonData.Instance.UNLOADER_SUBCARRIERID = sSUBCARRIERID;

                                    msg.AddAscii(sSUBCARRIERID);
                                    msg.AddAscii(sCELLQTY);
                                    int nCellQty = Convert.ToInt32(sCELLQTY);
                                    List<CELLINFO> CellInfoList = CommonData.Instance.GetCarrierCells(module.MODULE_NAME, "UnloaderSubCarrierID");
                                    msg.AddList(nCellQty);
                                    {
                                        for (int j = 0; j < nCellQty; j++)
                                        {
                                            sCELLID = CellInfoList[j].CELLID;
                                            sLOCATIONNO = CellInfoList[j].LOCATION;
                                            sJUDGE = CellInfoList[j].JUDGE;
                                            sREASONCODE = CellInfoList[j].REASONCODE;
            ...
            CommonData.Instance.OnStreamFunctionAdd("UNLOADER", "E->H", "MARTERIAL", "S6F11", sCEID, Materiallog_name, null);
            #endregion
            #endregion
        }
    }
