...
{
    class S6F11_UnitOPCALLConfirm_CEID513 : SFMessage
    {
        /*S6F11(UnitOPCALLConfirm), CEID :513*/
        private string _mode;

        public S6F11_UnitOPCALLConfirm_CEID513(SECSDriverBase driver, string Mode)
            : base(driver)
        {
        ...
        public override void DoWork(string driverName, object obj)
        {
            ...
            if(_mode == "UNIT")
            {
                cellIDTagName = string.Format("i{0}.OpcallConfirm.CellID", data.Module);
                productIDTagName = string.Format("i{0}.OpcallConfirm.ProductID", data.Module);
                stepIDTagName = string.Format("i{0}.OpcallConfirm.StepID", data.Module);
            }
            else
            {
                cellIDTagName = string.Format("i{0}.EQPOpcallConfirm.CellID", data.Module);
                productIDTagName = string.Format("i{0}.EQPOpcallConfirm.ProductID", data.Module);
                stepIDTagName = string.Format("i{0}.EQPOpcallConfirm.StepID", data.Module);
            }
            ...
            if (_mode == "UNIT")
            {
                ...
                {
                    opcallIDTagName[i] = string.Format("i{0}.OpcallConfirm.OpcallIDID", data.Module);
                    messageTagName[i] = string.Format("i{0}.OpcallConfirm.Message", data.Module);
                }
            }
            else
            {
                for (int i = 0; i < nOPCALL_NO; i++)
                {
                    opcallIDTagName[i] = string.Format("i{0}.EQPOpcallConfirm.OpcallIDID", data.Module);
                    messageTagName[i] = string.Format("i{0}.EQPOpcallConfirm.Message", data.Module);
                }
            }
            ...
            CommonData.Instance.OnStreamFunctionAdd(sMODULEID, "E->H", "OPCALL", "S6F11", "513", "Unit Opicall Confirm Send", null);
            #endregion
        }
    }
}